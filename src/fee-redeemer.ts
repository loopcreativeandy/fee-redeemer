...
export function solForEmptyAccounts(emptyAccounts: EmptyAccount[]): number {
  return (
    emptyAccounts
      ...
      .reduce((prev, curr) => {
        return prev + curr;
      }, 0) / sweb3.LAMPORTS_PER_SOL
  );
}

export function getPKsToClose(
...
export async function getTotalRedemptions(
  ...
  account: sweb3.PublicKey,
): Promise<TotalRedemptions | null> {
...
export async function createCloseEmptyAccountsTransactions(
  ...
  donationAddress?: sweb3.PublicKey,
): Promise<sweb3.Transaction[]> {
  const closeInstructions = accountPKs.map((accPK) =>
  ...
  while (closeInstructions.length > 0) {
    ...
    if (donationPercentage && donationAddress) {
      ...
      const donationAmount =
        (RENT_PER_TOKEN_ACCOUNT_IN_SOL * closeInstrCnt * donationPercentage) /
        100;
